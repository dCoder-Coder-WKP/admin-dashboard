...
export default function InlineExtraPrice({ extraId, initialPrice }: { extraId: string, initialPrice: number }) {
  ...
  const handleBlur = async () => {
    ...
    setStatus('saving');
    try {
      await updateExtraPrice(extraId, price);
      ...
      setStatus('error');
      toast.error('Failed to update extra price');
      setTimeout(() => setStatus('idle'), 1500);
    }
  };
